...
{
	map<int,bool>  a;
	a.insert(make_pair(1,true));
	a.insert(make_pair(2,true));
	map<int,bool> ::iterator iter(a.begin());
	map<int,bool>  ::iterator end_iter(a.end());
	map<int,bool>  ::iterator m_ProcessNextIter;


	bool bHandled = false;
	while(iter != end_iter)
	{
		map<int,bool> ::iterator IterTemp = iter;
		m_ProcessNextIter = ++IterTemp;		
		a.erase(iter);
		iter = m_ProcessNextIter;
		continue;
	}
